function DFS(startNode, skipNframes, startColor, graph) {
  ...
  this.nextStep = function(){
    ...
    if(node.parent != null){
      if(random() < 0.2){
       node.color.x = node.parent.color.x * random(0.9999 , 1);
       node.color.y = node.parent.color.y * random(0.9999 , 1);
       node.color.z = node.parent.color.z * random(0.9999 , 1);
     }else{
    ...
    for(var i = 0; i < arr.length;i++){
      ...
      if(CheckBounds(tmp)){
        if(!this.graph[tmp.y][tmp.x].visited){
